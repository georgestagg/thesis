...
By noticing that $p = \frac{1}{2}g \left ( \frac{n}{m} \right ) ^2 = \frac{gR^4}{2}$ we can write $\frac{\partial p}{\partial x_k} = 2gR^3\frac{\partial R}{\partial x_k}$ and then,
\begin{equation*}
...
\end{equation*}
where $P_{jk} = -\frac{\hbar^2}{4m^2}n\frac{\partial^2\ln{n}}{\partial x_j \partial x_k}$. Writing this in vector notation, we obtain an equation similar to the Euler equation for an inviscid fluid,
\begin{equation}
...
\label{eq:Esplitdef}
\end{equation*}
by using $\sqrt{n}v_j = (\sqrt{n}v_j)^c + (\sqrt{n}v_j)^i$, with $\nabla(\sqrt{n}v_j)^i=0$, where $\mathbf{v}$ is the fluid velocity. The spectrum of $E^i_{\rm kin}$ can be defined so that a similar relation to Equation \ref{eq:Ekinspec} holds.
...
\end{equation}
Note that, in general, the healing length depends on the condensate density $n$. However, in a homogeneous system $n({\bf r},t) = \rho$ is the uniform density, and so we can write the healing length as
\begin{equation}
...
\end{equation}
where $n({\bf r},t) = |\psi({\bf r},t)|^2$ is the local condensate density, $p_e$ is the momentum of elementary excitations and $E(p_e)$ their energy \cite{NozieresPines}. Consider excitations with a small momentum $p_e$. Here the dispersion relation is linear,
\begin{equation}
E(p_e)\approx \sqrt{\frac{ng}{m}}p_e ,
\end{equation}
and becomes sound-like, with a velocity $c_{\rm local} = \sqrt{ng/m}$ dependent on the local condensate density $n({\bf r},t)$. In a homogeneous system $n({\bf r},t) = \rho$ is the uniform density, and so the speed of sound in a homogeneous system is
\begin{equation}
c = \sqrt{\frac{\rho g}{m}}.
\end{equation}
