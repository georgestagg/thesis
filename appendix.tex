...
\label{eq:Esplitdef}
\end{equation}
The kinetic energy can be further decomposed into compressible and incompressible parts by considering the kinetic energy spectrum $\hat{E}_{\rm kin}$ \cite{Nore},
\begin{equation}
...
\begin{algorithm}
  ...
  \For{$i\leftarrow 1+l/2$ \KwTo $n_x-l/2$}{
    ...
    \If {$V[i,j]>1$}{$Q[i,j]\leftarrow0$\tcp*{Avoid detection of ghost vortices}} 
  }
