...
\centering
  \begin{tikzpicture}[domain=-5:5]
    ...
    \end{axis}
  \end{tikzpicture}%
\caption{A sample RCA decompotion into clusters, free vortices and dipoles (green). Positive (negative) clusters and vortices are shown as blue triangles (red circles).}\label{fig:rca}
\end{figure}
...
\end{equation}
where $C$ is the a set of vortices in a particular cluster, $N_C$ is the number of vortices in the set $C$, and ${\rm r}_i$ is the vortex location. Additionally, the size of a particular cluster can be quantified using an expression for the approimate cluster radius,
\begin{equation}\label{eq:rcacom}
