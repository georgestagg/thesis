...
Vortices can be tracked in space over several time points, leads to the building of vortex trajectories. The process of building trajectories from the location of many particles over time is known as particle linking. Note that although vortex location is known, their identity is unknown and so it is not obvious what vortex at one point in time corresponds to another at a different time.

The first step is to perform frame-to-frame particle linking. Trajectories are built by moving from one time point to the next and linking the vortex pairs that are closest together (taking into account euclidean distance and vortex charge), as these are most likely to be the same vortex. This is performed efficiently using the Hungarian algorithm [CITE], an algorithm that links many pairs of particles at two different time points by minimising the sum of all the pair distances.

As the simulation develops, the number of vortices will change and so trajectories will end early. A threshold value in the Hungarian algorithm controls when a track ends. If the distance between a pair of vortices exceeds the threshold, it is assumed they cannot possibly be the same vortex, they are not linked, and instead the trajectory is ended.

Sometimes, due to numerical error or simulation dynamics, a vortex will not be detected at one or more time points before reappearing. The particle linking algorithm must take this into account as the second step. The second step is to iterate once more through the data and investigate the ends of the trajectories. If a track beginning is found close (once again taking into account euclidean distance and vortex charge) to a track end within a certain number of time points, a link is made to join the two trajectories. Once again a threshold value for distance between points and distance in time is employed. Should the tracks be too far apart in space or time, it is assumed they are {\it not} the same vortex and remain unlinked.
\begin{figure}[!ht]
\begin{center}
   \begin{tikzpicture}
  ...
  \end{tikzpicture}%
  \end{center}
  \caption{FIGURE TO BE ADDED}\label{fig:vortextracks}
\end{figure}
