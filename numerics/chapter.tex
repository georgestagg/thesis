...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Vortex field for two oppositely charged vortices located at ($-\xi$,0) and ($\xi$,0). The positive (negative) vortex leads to a positive (negative) quantity in the vortex field. In addition, the output of the bwlabel algorithm is labelled.\label{fig:vortexfield}}
 \end{figure}

For every vortex detected by this algorithm, the corresponding area in the vortex field is composed of several adjoined points in the numerical grid. For correct vortex counting, these multiple points must be classified as a single vortex. The bwlabel algorithm described in Algorithm \ref{algo_bwlabel} is used to obtain the required classification. The algorithm takes the vortex field as input and assigns each connected component a label. An example output of bwlabel is shown in Figure \ref{fig:vortexfield}. To find the vortex location for a given label, we simply take the mean of all the $x$ and $y$ locations of points with the same label.
...
When using Algorithm \ref{algo_calcvortexfield} in `messy' wavefunctions (containing many vortices, potentials and waves), artefacts of the incorrect sign can be introduced by the numerical discretisation, differentiation and integration errors. Faint examples of these artefacts can be seen in Figure \ref{fig:vortexfield} surrounding the two labelled regions. These artefacts spatially small (on the scale of $\Delta_x$) and so can be corrected by removing all high frequency spacial waves from the vortex field using low-pass filtering. A common low-pass filter is the Gaussian blur, applied using a convolution. The Gaussian blur algorithm is described in Algorithm \ref{algo_gaussconv} and the result of applying the algorithm to Figure \ref{fig:vortexfield} is shown in Figure \ref{fig:vortexfieldsmooth}. Note how the incorrectly signed artefacts surrounding the detected vortex regions are now removed.
\begin{figure}[!ht]
  \centering
   \begin{tikzpicture}
      \begin{axis}[
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Smoothed vortex field for two oppositely charged vortices located at ($-\xi$,0) and ($\xi$,0). The high frequency noisy artefacts are removed by the low-pass filtering.\label{fig:vortexfieldsmooth} }
 \end{figure}

 A by-product of applying a low-pass filter to the vortex field is that sharp edges in structures are blurred and spread out. To identify vortex locations a threshold function is therefore used before performing the bwlabel algorithm, i.e vortices are identified when $|\Delta\theta(C_{[i,j]})| > \Delta_{\rm th}$, where $\Delta_{\rm th}>0$ is some threshold depending on the exact system used. $\Delta_{\rm th}$ can be tweaked to make the vortex detection more or less sensitive (for either detecting vortices more easily in messy systems, or to ignore spurious signals in the vortex field).
\subsection{\label{section:ghostvortex} Avoiding `ghost vortices'}
A common problem when identifying vortex locations is the prevalence of invalid or uninteresting phase defects inside obstacles and when considering a trapped condensate. In areas where the density is zero the system phase becomes undefined, depending on how the simulation is implemented these areas may fill with numerical noise and cause many singularities to be detected. A similar phenomenon occurs in areas of near-zero density. Well-defined singularities in the phase form but with an ill-defined vortex core in the density. Examples of these vortices are shown in the low density regions of Figure \ref{fig:ghostvortex}; as these singularities carry negligible angular momentum and energy, they are of no interest and are termed `ghost vortices'\cite{tsubota_kasamatsu_02}.
\begin{figure}[!ht]
  \centering
   \begin{tikzpicture}
      \begin{axis}[
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{A trapped rotating condensate with $\Omega=0.7$, $\hat{g}=2000$ and $\hat{\mu}=25.27$. Ghost vortices can be seen in the phase as singularity where the corresponding density is near-zero.\label{fig:ghostvortex}}
 \end{figure}

A simple way to remove ghost vortices is to use a mask to hide vortices found in the unimportant phase regions in trapped systems and inside obstacles. While on the one hand it is easy to define a mask using the TF radius or where $V_{\rm trap} + V_{\rm obj}>1$, masks still are quite arbitrary in nature and an overzealous mask may hide details. Additionally, any condensate with a breathing mode or centre of mass oscillation will periodically extend beyond a hard coded mask.

An alternate method presents itself when implementing Algorithm \ref{algo_calcvortexfield}. The vortex field can be multiplied by the wavefunction density (as a fraction of its maximal value) at every point {\it before} performing Gaussian convolution. The result is that in low-density areas the vortex field becomes completely zero and are no longer identified. At the centre of vortex cores the vortex field also becomes zero. However, enough vortex information remains {\it around} vortex cores that after the Gaussian convolution vortices in high-density regions are still identified accurately. The threshold value $\Delta_{\rm th}$ now controls the sensitivity of the algorithm to vortices in the low density regions of the condensate. Algorithm \ref{algo_calcvortexfielddens} describes this method in further detail. Figure \ref{fig:filtervortexlattice} demonstrates the vortex field with and without multiplying by the density before the high-frequency filter. The ghost vortices detected when using Algorithm \ref{algo_calcvortexfield} are indeed removed when using Algorithm \ref{algo_calcvortexfielddens}.
\begin{figure}[!ht]
  \centering
   \begin{tikzpicture}
      \begin{axis}[
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{A trapped rotating condensate with $\Omega=0.7$, $\hat{g}=2000$ and $\hat{\mu}=25.27$. (a) The smoothed vortex field using Algorithm \ref{algo_calcvortexfield}, (b) the smoothed vortex field using Algorithm \ref{algo_calcvortexfielddens}. The ghost vortices seen in (a) are removed in (b).\label{fig:filtervortexlattice}}
 \end{figure}
