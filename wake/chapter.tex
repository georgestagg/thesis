...
The 2D (3D) system is simulated using the fourth-order Runge-Kutta method described in Section \ref{section:RK4} under periodic boundary conditions on a $2048 \times 512$ ($400 \times 150 \times 150$) grid with uniform spacing $\Delta_x=0.4\xi$. Phase unwinding (described in Section \ref{section:vortexremoval}) is used in combination with a sufficiently large computational box so that the boundary conditions and nucleated vortices do not play a role in further vortex shedding.  The initial condition is the stationary state of the GPE (including obstacle potential) with $v=0$ as determined by the imaginary time convergence method described in Section \ref{section:imagTime}. Setting $V_0=100\,\mu$ throughout, the external potential closely approximates an impenetrable obstacle. A small amount of noise is added to the initial condition to break symmetry: Both the real and imaginary parts of the initial wavefunction are perturbed at every grid point by random numbers of size $0.1\%$. To minimize initial generation of waves, $v$ is ramped up in time along a hyperbolic tangent curve, from $v=0$ at $t=0$ to its terminal value at $t\approx200~(\xi/c)$.
...
\begin{tikzpicture}
  \begin{axis}[ylabel near ticks,xlabel near ticks,
    ...
    \addplot graphics [xmin=311.2,xmax=408.8,ymin=-20,ymax=20] {wake/quiver2};
  \end{axis}
\end{tikzpicture}
\begin{tikzpicture}
  \begin{axis}[ylabel near ticks,xlabel near ticks,
...
For a circular object, we find that the critical velocity is $v_{\rm c}=0.355\pm 0.005~c$, consistent with predictions in the Eulerian ($d \gg \xi$) limit \cite{berloff2000,rica2001,pham2004}.

Studies on circular objects show that $v_{\rm c}$ depends on the obstacle's shape at small diameters, where boundary effects are significant; $v_{\rm c}$ approaches the ``Eulerian" value only for large diameters $d \gg \xi$ \cite{huepe00,rica2001}.  The variation of $v_{\rm c}$ with the obstacle width $d$ is shown in Figure \ref{fig:velplots} (b).  For $d=10\xi$, the critical velocity effectively reaches its asymptotic value, while at smaller widths, it is much larger.  As the ellipticity is increased (i.e. the obstacle becomes narrower in $x$), the critical velocity decreases.  The modification of the critical velocity is significant: if $\varepsilon=3$, $v_{\rm c}$ is more than $40\%$ smaller than that for a circular obstacle. 
...
\subsection{A `superfluid Reynolds number'}
The work in this chapter has been built upon by Reeves \emph{et. al.} \cite{reeves_2015}. The group performed 2D simulations using circular ($\varepsilon=1$) obstacles over a large parameter space of $d$ and $v$. The group performed many GPE simulations using a high speed CUDA code running on a cluster of graphics cards. The large number of simulations enabled Reeves \emph{et. al.} to identify a superfluid Reynolds number, via dynamical similarity of a hard cylinder in classical viscous flow and a soft Gaussian obstacle in superfluid flow.

\begin{table}
\vspace{-0.25cm}
\centering
\begin{tabular}{ccccc}
$v/c$ & ${\rm Re}_s$ & $f\tau$ & ${\rm St}$ & Predicted ${\rm St}$ \\
\hline \\ [-2ex]
$0.500$ & $3.004$ & $3.053 \times 10^{-3}$& $0.1310$ & $0.1353$\\
$0.450$ & $1.931$ & $3.187 \times 10^{-3}$& $0.1519$ & $0.1343$\\
$0.400$ & $0.8584$ & $2.700 \times 10^{-3}$& $0.1363$ & $0.1259$\\
$0.365$ & $0.1073$ & $1.329 \times 10^{-3}$& $0.0781$ & $0.0955$\\
\hline
\end{tabular}
\caption{Measurements of quantities related to the calculation of the superfluid Reynolds number, ${\rm Re}_s$, from a selection of our simulations with $\varepsilon=1$ and $d=5\xi$. Our measurements of ${\rm St}$ agree well with the best fit ${\rm St}-{\rm Re}_s$ curve calculated in \cite{reeves_2015}.}
\label{tbl:re_st}
\end{table}


Reeves \emph{et. al.} define the superfluid Reynolds number as, ${\rm Re}_s = (v-v_c)D/\kappa$, where $D = 2d[\log(V_0/\mu)]^{1/2}$ is the effective cylinder width and $\kappa=\hbar/m$. For circular obstacles, ${\rm Re}_s$ characterises the transition to quantum turbulence, with the predicted development of an irregular wake at ${\rm Re}_s \approx 0.7$, irrespective of cylinder size. The authors go on to measure the Strouhal number, ${\rm St} = fD/v$, where $f$ is the dominant frequency in the power spectrum of $F_y$, the transverse force on the obstacle (defined in Appendix \ref{appsection:force}). It was found that the Strouhal and superfluid Reynolds numbers lie on a classical-like ${\rm St}-{\rm Re}_s$ curve of the form ${\rm St} = {\rm St}_{\infty}[1-\alpha/({\rm Re}_s+\beta)]$. We have confirmed that our simulations with circular obstacles agree with this observation and are reasonably consistent with the best fitting ${\rm St}-{\rm Re}_s$ curve obtained by Reeves \emph{et. al.}, ${\rm St} = 0.1402[1-0.1126/({\rm Re}_s+0.2456)]$. A selection of our measurements are listed, along with the predicted values of ${\rm St}$, in Table \ref{tbl:re_st}.
\end{chapter}
