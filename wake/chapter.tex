...
While true for incompressible potential flow, the GPE describes a compressible fluid, so the local density (and therefore speed of sound), $n(\bf r) = |\psi|^2$, in regions of high velocity is reduced according to Bernoilli's equation (ignoring the quantum pressure) \cite{win01},
\begin{equation}
